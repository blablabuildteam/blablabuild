...
export const LEVEL2_SEED_VERSION = 20;
...
export const LEVEL2_SEEDS: Record<string, Seed> = {
  ...
  yluy9f0i: {
    ...
    idea:
      'First make the existing work a real skill, not a chat. Package it: SKILL.md, the scorecard / must-haves, example CVs they already used, and a shortlist that felt right. Then grow that same skill so a shortlisted candidate also gets an interview kit that leans on Adsomnia’s edge: culture, current teams and departments, and soft-skill probes — not another pass at the public job ad (candidates can prep that the same way). Hard-skill fit vs the website posting still happens; it is the floor, not the interview. Prove it on one past or hypothetical applicant. No TeamTailor, no LinkedIn, no live ATS. Enneagram / team-gap synergy hiring is a later scale, not this sprint.',
    instructions:
      '1. Package the chat first. If this still lives in a session, turn it into a skill folder: SKILL.md, the role/scorecard they used, two example CVs, and one shortlist they would stand behind. A colleague should be able to run a dry screen from the files alone.\n\n2. Put Adsomnia culture and current team/department notes into that same skill — the private context a candidate cannot scrape from the job page.\n\n3. Add interview prep. Hard-skill match vs the public posting is one output. The kit the interviewer walks in with should zoom in on soft skills, culture, and how they would sit with this team. Do not spend the interview re-testing the JD.\n\n4. Run it once on a past or made-up applicant. Filter, then produce both the hard-skill shortlist and the soft-skill kit. Ask: would an interviewer walk in with this?\n\n5. Keep CVs as paste or export in their own Claude project. No TeamTailor dump, no live ATS, no sharing candidate files outside HR. Do not collect Enneagram profiles this sprint.',
    presentationExpect:
      'Show that it is possible:\n\n1. The skill folder — not the original chat — on screen, including culture / team notes (not only the JD).\n2. A shortlist from the hard-skill filter (one past or hypothetical pack is enough).\n3. The interview kit for 1–2 of those candidates — clearly weighted to soft skills and culture, not a recap of the job ad.\n4. A yes/no: would an interviewer walk in with this? If yes, the case is proven.\n\nOut of scope this sprint: live hiring volume, TeamTailor or LinkedIn, a GDPR platform decision, becoming an ATS, Enneagram profiles of current teammates. Those come after a successful proof — and when there are actual openings.',
    crossTeams: [],
    prioritizeReuse: '',
    status: 'ready',
  },
